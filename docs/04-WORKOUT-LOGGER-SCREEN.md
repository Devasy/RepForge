# Fix #4: Implement Functional Workout Logger Screen

## Problem
Current `WorkoutLoggerScreen` has:
- Hardcoded exercises array
- Non-functional input fields
- No database integration
- Cannot actually log workouts
- No ability to add/remove exercises or sets

## Solution: Connect to Context and Make Interactive

### Implementation Steps

#### Update WorkoutLoggerScreen.js

Replace the entire file with functional implementation:
        ...
        {exercise.sets.map((set, index) => (
          <View key={set.id} style={styles.setRow}>
            ...
            <TextInput
              ...
              onChangeText={(value) => onUpdateSet(set.id, { weight_lbs: parseFloat(value) || 0 })}
            ...
            <TextInput
              ...
              onChangeText={(value) => onUpdateSet(set.id, { reps: parseInt(value) || 0 })}
              keyboardType="numeric"
              placeholder="0"
              placeholderTextColor="#929bc9"
            />
...
const WorkoutLoggerScreen = ({ navigation }) => {
  ...
  // Start workout on screen load if not active
  useEffect(() => {
    if (!isWorkoutActive) {
      const name = `Workout - ${new Date().toLocaleDateString()}`;
      setWorkoutName(name);
      handleStartWorkout(name);
    }
  }, []);
