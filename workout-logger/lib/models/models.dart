...
enum SuggestionType { reorder, replace, add }

class RoutineSuggestion {
  final SuggestionType type;
  final String reasoning;

  /// reorder only: full new ordered list of the routine's existing exercise IDs.
  final List<String>? reorderedExerciseIds;

  /// replace only: ID of the exercise to remove from the routine.
  final String? removeExerciseId;

  /// replace only: AI-supplied name of the replacement exercise.
  final String? replaceWithName;

  /// replace only: resolved exercise ID (filled by ViewModel after parsing).
  String? replaceWithId;

  /// add only: AI-supplied name of the exercise to append.
  final String? addExerciseName;

  /// add only: resolved exercise ID (filled by ViewModel after parsing).
  String? addExerciseId;

  RoutineSuggestion({
    required this.type,
    required this.reasoning,
    this.reorderedExerciseIds,
    this.removeExerciseId,
    this.replaceWithName,
    this.replaceWithId,
    this.addExerciseName,
    this.addExerciseId,
  });

  factory RoutineSuggestion.fromJson(Map<String, dynamic> j) =>
      RoutineSuggestion(
        type: SuggestionType.values.byName(j['type'] as String),
        reasoning: j['reasoning'] as String? ?? '',
        reorderedExerciseIds:
            (j['reordered_exercise_ids'] as List?)?.cast<String>(),
        removeExerciseId: j['remove_exercise_id'] as String?,
        replaceWithName: j['replace_with_name'] as String?,
        addExerciseName: j['add_exercise_name'] as String?,
      );
}

class RoutineOptimizationResult {
  final String summary;
  final List<RoutineSuggestion> suggestions;
  ...
  factory RoutineOptimizationResult.fromJson(Map<String, dynamic> j) =>
      RoutineOptimizationResult(
        summary: j['summary'] as String? ?? '',
        suggestions: (j['suggestions'] as List? ?? [])
            .map((s) => RoutineSuggestion.fromJson(s as Map<String, dynamic>))
            .toList(),
      );
}
